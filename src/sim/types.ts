...
/** Two forms of "politics" with a real, mechanical effect: banishing an existing member, or
 *  admitting a candidate waiting in the "waiting room" (see Agent.pending). Same vote mechanic
 *  either way — majority of the current active town, real and permanent either way it resolves.
 *  Not a general law-making system, just these two specific, high-stakes votes. */
export type ProposalKind = 'banish' | 'admit';
...
export interface Agent {
  ...
  model: string;
  /** True while waiting on an admission vote — not in agentOrder yet, so it's never planned,
   *  ticked, or rendered; entirely inert until the town votes it in (or rejects it, at which
   *  point it's removed). Only agents added to an already-populated town go through this; the
   *  founding roster and lineage-born children spawn active immediately, nothing to admit them
   *  into yet or from. */
  pending: boolean;
  color: string;
